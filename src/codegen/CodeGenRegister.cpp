...
string CodeGenRegister::value2reg(Value *v, int i, string recommend) {
  ...
  if (dynamic_cast<Constant *>(v)) {
    ...
    makeSureInRange("addi.d", reg_name, FP, context.offset_map.at(v), "add.d",
                    i);
    ...
    makeSureInRange(instr_ir + suff, reg_name, FP, context.offset_map.at(v),
                    instr_ir + "x" + suff, i);
  }
  return reg_name;
}
...
void CodeGenRegister::ptrContent2reg(Value *ptr, string dest_reg) {
  ...
  if (dynamic_cast<GlobalVariable *>(ptr)) {
    ...
    makeSureInRange(instr_ir + suff, dest_reg, FP, context.offset_map.at(ptr),
                    instr_ir + "x" + suff);
...
void CodeGenRegister::gen_call() {
  ...
  for (auto [op, reg, off] : store_record) {
    ...
    makeSureInRange(instr_ir + suff, reg, SP, totalN - off,
                    instr_ir + "x" + suff);
  }
  // pass arguments
  ...
  for (auto &func : m->get_functions()) {
    if (not func.is_declaration()) {
      ...
      for (auto [_, op] : LRA.get()) {
        auto regmap =
            op->get_type()->is_float_type() ? RA_float.get() : RA_int.get();
