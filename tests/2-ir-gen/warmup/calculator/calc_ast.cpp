...
CalcASTNode *CalcAST::transform_node_iter(syntax_tree_node *n) {
  if (_STR_EQ(n->name, "input")) {
    ...
    auto expr_node =
        static_cast<CalcASTExpression *>(transform_node_iter(n->children[0]));
    ...
    if (n->children_num == 3) {
      ...
      auto factor_node =
          static_cast<CalcASTFactor *>(transform_node_iter(n->children[2]));
      node->factor = std::shared_ptr<CalcASTFactor>(factor_node);
    } else {
      auto factor_node =
          static_cast<CalcASTFactor *>(transform_node_iter(n->children[0]));
      node->factor = std::shared_ptr<CalcASTFactor>(factor_node);
    }
    return node;
