...
int main() {
  ...
  // 用 builder 创建 getelementptr 指令，参数解释：第一个参数传入 getelementptr
  ...
  // 用 builder 创建一条条件跳转指令，使用 icmp 指令的返回值作为条件，trueBB 与
  // falseBB 作为条件跳转的两个目的 basicblock
  builder->create_cond_br(icmp, trueBB, falseBB);
  // 将 builder 插入指令的位置调整至 trueBB 上
  builder->set_insert_point(trueBB);
  // 用 builder 创建 store 指令，将 a 的值存到 temp 处，将 b 存到 a 处，将 temp
  // 重新 load 后存到 b 处，对应源代码 temp=a，a=b，b=temp
  ...
  // 用 builder 创建 getelementptr 指令，参数解释：第一个参数传入 getelementptr
  // 的指针参数，第二个参数传入取偏移的数组。在此条语句中，x 为 [1 x
  // i32]*类型的指针，取两次偏移，两次偏移值都为 0，因此返回的即为 x[0] 的地址。
